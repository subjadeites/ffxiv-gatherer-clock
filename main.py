...
def clock_out(lang, Eorzea_time_in, need_tts, func, ZhiYe, lvl_min, lvl_max, choose_DLC, client_verion,
    ...
    if choose_DLC == '自定义筛选':
        out_list = []
        select = "(clock['开始ET'] <= Eorzea_time_in) & (clock['结束ET'] > Eorzea_time_in)" + '&clock.材料名' + lang + '.isin(more_select_result)'
        clock_found = clock[eval(select)].sort_values(by='等级',ascending=False).head(None)
        out_list_next = []
        select_next = "(clock['开始ET'] <= next_start_time) & (clock['结束ET'] > next_start_time)" + '&clock.材料名' + lang + '.isin(more_select_result)'
        ...
        select = "(clock['开始ET'] <= Eorzea_time_in) & (clock['结束ET'] > Eorzea_time_in)" + func_select(
            ...
            ZhiYe) + "& (clock['等级'] <= " + str(lvl_max) + ")" + "& (clock['等级'] >= " + str(
            lvl_min) + ")" + choose_DLC_dict.get(choose_DLC)
        clock_found = clock[eval(select)].sort_values(by='等级',ascending=False).head(None)
        # 这部分是用于预告下一次刷新的代码
        out_list_next = []
    ...
    if len(clock_found) == 0 and len(clock_found_next) == 0:
        ...
        for i in range(0, len(clock_found)):
            temp_out_list = [clock_found.iloc[i]['材料名' + lang], str(int(clock_found.iloc[i]['等级'])),
                             clock_found.iloc[i]['职能'],
                             clock_found.iloc[i]['类型'], clock_found.iloc[i][place_keyword], clock_found.iloc[i]['靠近水晶'],
        ...
        for i in range(0, len(clock_found_next)):
            temp_out_list_2 = [clock_found_next.iloc[i]['材料名' + lang], str(int(clock_found_next.iloc[i]['等级'])),
                               clock_found_next.iloc[i]['职能'],
                               clock_found_next.iloc[i]['类型'], clock_found_next.iloc[i][place_keyword],
        ...
        if need_tts is True:
            ...
            for i in range(0, len(out_list)):
                nearby = '' if out_list[i][5] == '无数据' else out_list[i][5]
