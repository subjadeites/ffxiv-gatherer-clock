...
class Loading_Windows(wx.Frame):
    def __init__(self, parent, title=""):
        super().__init__(parent=None, title=title, size=(300, 170), style=wx.SIMPLE_BORDER | wx.TRANSPARENT_WINDOW | wx.NO_BORDER)  # 继承wx.Frame类
    ...
    def check_csv_func(self,event):
        try:
            ...
            if public.clock is not None:
                ...
                wx.MessageDialog(None, "无法读取csv文件，请检查csv文件是否存在或者是否被占用", "错误", wx.OK | wx.ICON_ERROR).ShowModal()
                self.Destroy()
                exit()
        except :
            pass
